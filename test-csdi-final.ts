 ...
 * Usando crypto.createPrivateKey con formato DER
 * 
 * Uso: npx tsx test-csdi-final.ts
 */
...
<cfdi:Comprobante 
  ...
  <cfdi:Receptor 
    Rfc="VIPE9401251Z5" 
    Nombre="ESMERALDA VICENCIO PADRON" 
    DomicilioFiscalReceptor="32320" 
  ...
  <cfdi:Conceptos>
    <cfdi:Concepto 
      ...
      Cantidad="1" 
      Unidad="SERVICIO" 
      NoIdentificacion="SER004" 
      Descripcion="HONORARIOS CONTABLES" 
      ValorUnitario="862.07" 
      Importe="862.07" 
      ObjetoImp="02">
      <cfdi:Impuestos>
        <cfdi:Traslados>
...
async function sellarCFDI(): Promise<{ success: boolean; xmlSellado?: string; error?: string }> {
  try {
    ...
    // Extraer número de certificado del DER
    let noCertificado = '';
    try {
      // El número de certificado está en bytes específicos del DER
      // Para CSD del SAT, generalmente está en una posición fija
      // Usamos el nombre del archivo como fallback
      const fileName = path.basename(CER_PATH, '.cer');
      noCertificado = fileName;
      console.log('🔢 No. Certificado (del nombre de archivo):', noCertificado);
    } catch (e) {
...
// ============ EJECUCIÓN PRINCIPAL ============
async function main() {
  console.log('🚀 [Test CSD FINAL] Iniciando prueba de sellado...\n');
  console.log('💡 Usando crypto.createPrivateKey con formato DER\n');
  ...
  if (!resultado.success) {
    ...
    console.log('   3. Que el archivo .key es formato PKCS#8 DER encriptado');
    return;
  }

  console.log('\n✅ [Test CSD FINAL] CFDI sellado exitosamente!\n');
  console.log('📊 Resumen:');
  ...
  console.log('\n💾 [Test CSD FINAL] Request SOAP guardado en:', soapPath);

  console.log('\n✅ [Test CSD FINAL] ¡Listo para probar en SoapUI!');
  console.log('\n📋 Instrucciones:');
