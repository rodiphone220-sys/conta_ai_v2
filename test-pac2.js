// Test PAC endpoint simple
import fetch from 'node-fetch';

const endpoint = 'https://testing.solucionfactible.com/ws/services/Timbrado';
const user = 'testing@solucionfactible.com';
const password = 'timbrado.SF.16672';

// CFDI 4.0 válido con RFC de pruebas de Solucion Factible
const testCfdiBase64 = '';

const SOAP_NAMESPACE = 'http://timbrado.ws.cfdi.solucionfactible.com';

const soapRequest = `<?xml version="1.0" encoding="UTF-8"?>
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ws="${SOAP_NAMESPACE}">
  <soapenv:Header/>
  <soapenv:Body>
    <ws:timbrar>
      <ws:usuario>${user}</ws:usuario>
      <ws:password>${password}</ws:password>
      <ws:cfdiBase64>${testCfdiBase64}</ws:cfdiBase64>
    </ws:timbrar>
  </soapenv:Body>
</soapenv:Envelope>`;

async function test() {
  try {
    console.log('Enviando request SOAP a:', endpoint);
    console.log('Namespace:', SOAP_NAMESPACE);
    const response = await fetch(endpoint, {
      method: 'POST',
      headers: {
        'Content-Type': 'text/xml; charset=utf-8',
        'SOAPAction': ''
      },
      body: soapRequest
    });

    console.log('Status:', response.status, response.statusText);
    const text = await response.text();
    console.log('Response:', text);

    if (text.includes('<uuid>')) {
      const uuidMatch = text.match(/<uuid>([^<]+)<\/uuid>/i);
      console.log('UUID:', uuidMatch ? uuidMatch[1] : 'No encontrado');
    }
  } catch (error) {
    console.error('Error:', error);
  }
}

test();
