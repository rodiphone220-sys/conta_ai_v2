/**
 * Script alternativo para sellar CFDI usando OpenSSL
 * para probar en SoapUI con Solución Factible
 * 
 * Uso: npx tsx test-csdi-openssl.ts
 */
...
const TEMP_DIR = path.join(process.cwd(), 'temp-csd');
...
function prepararTempDir() {
  if (!fs.existsSync(TEMP_DIR)) {
    fs.mkdirSync(TEMP_DIR, { recursive: true });
  }
}

// ============ CONVERTIR KEY DER A PEM ============
function convertirKeyDerAPem(): string {
  const keyPemPath = path.join(TEMP_DIR, 'llave.pem');
  
  try {
    // Usar OpenSSL para convertir DER a PEM
    const opensslCmd = `openssl pkcs8 -inform DER -in "${KEY_PATH}" -passin pass:${CSD_PASSWORD} -out "${keyPemPath}" -nocrypt`;
    console.log('🔧 Ejecutando:', opensslCmd);
    execSync(opensslCmd, { stdio: 'pipe' });
    
    console.log('✅ Llave convertida a PEM');
    return fs.readFileSync(keyPemPath, 'utf-8');
  } catch (error: any) {
    console.error('❌ Error al convertir llave:', error.message);
    console.log('💡 Nota: Necesitas tener OpenSSL instalado');
    console.log('💡 Descarga OpenSSL de: https://slproweb.com/products/Win32OpenSSL.html');
    throw new Error('OpenSSL no disponible o contraseña incorrecta');
  }
}
...
async function sellarCFDI(): Promise<{ success: boolean; xmlSellado?: string; error?: string }> {
  try {
    ...
    const cadenaPath = path.join(TEMP_DIR, 'cadena_original.txt');
    fs.writeFileSync(cadenaPath, cadenaOriginal, 'utf-8');
    console.log('💾 Cadena original guardada en:', cadenaPath);

    // Convertir llave DER a PEM
    console.log('🔄 Convirtiendo llave DER a PEM...');
    const keyPem = convertirKeyDerAPem();
...
// ============ EJECUCIÓN PRINCIPAL ============
async function main() {
  console.log('🚀 [Test CSD OpenSSL] Iniciando prueba de sellado...\n');
  ...
  if (!resultado.success) {
    ...
    console.log('\n💡 Verifica:');
    console.log('   1. Que OpenSSL está instalado (ejecuta: openssl version)');
    console.log('   2. Que la contraseña es correcta');
    console.log('   3. Que el CSD está vigente (no expirado)');
    return;
  }

  console.log('\n✅ [Test CSD OpenSSL] CFDI sellado exitosamente!\n');
  console.log('📊 Resumen:');
  ...
  console.log('\n💾 [Test CSD OpenSSL] Request SOAP guardado en:', soapPath);

  console.log('\n✅ [Test CSD OpenSSL] ¡Listo para probar en SoapUI!');
  console.log('\n📋 Instrucciones:');
